...
import { createTypeScriptResource, TypeScriptResourceConfig } from './typescript-resource-factory.js';
...
const DEFAULT_DESCRIPTIONS: Record<string, string> = {
  ...
  'sample2': 'Advanced TypeScript patterns with async/await, enums, conditional types, and error handling',
  'react-sample': 'React TypeScript component with JSX, hooks, event handling, and modern React patterns',
  ...
  try {
    ...
    for (const file of tsFiles) {
      ...
      try {
        ...
        console.log(`✓ Auto-discovered TypeScript resource: dbk-ts://${config.uriId} (${config.name})`);
